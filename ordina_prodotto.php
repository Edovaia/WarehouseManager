<?php
session_start();
if(!isset($_SESSION["admin"])){
 header('location:login.php');
}else{
   ?>
       <!DOCTYPE html>
    <html lang="it">
        <head>
                <meta charset="UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0">
                <title>Navbar Magazzino</title>
                
                <link rel="stylesheet" type="text/css" href="style_admin.css?ts=<?=time()?>&quot">
                <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
        
        </head>
    <nav class="navbar navbar-expand-lg bg-body-tertiary navbar" style="margin-top:-10px;">
  <div class="container-fluid">
  <a class="nav-link" aria-current="page" id="indietro" href="mostra_magazzino.php"> Indietro </a>
    <a class="nav-link" href="admin_home.php"><nobr><b>Scuderia Torino</b></nobr></a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav me-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link" aria-current="page" href="mostra_magazzino.php">Magazzino</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="modifica_magazzino.php">Modifica magazzino</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="stato_lavori.php">
           Lavori cliente
          </a>
         
        </li>
        <li class="nav-item">
        <?php 
          $conn=mysqli_connect("localhost","root","","scuderia torino") or die("Connessione fallita!");
          $xf="SELECT * FROM notifiche";
          $qf=mysqli_query($conn,$xf);
          if(mysqli_num_rows($qf)>0){
            $number=mysqli_num_rows($qf); ?>
            <a class="navbar-brand" href="notifiche.php">Notifiche</a><span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger"><?php echo $number;?> </span>
            <?php
          }else{
          ?>
          <a class="navbar-brand" href="notifiche.php">Notifiche</a>
          <?php
          }
          ?>
        </li>
        <li class="nav-item">
          <a class="nav-link logout" href="logout_admin.php">Logout</a>
        </li>
      </ul>
     
    </div>
  </div>
</nav>

   <body background="immagini\DSC_0474_PS_2_senza-scritta.jpg">
    
   <?php
   $conn=mysqli_connect("localhost","root","","scuderia torino");

   $x="SELECT nome_prodotto, azienda_produttrice, Email        
   FROM magazzino  
   INNER JOIN fornitori ON magazzino.fornitore=fornitori.Nome
   WHERE id_prodotto=$_POST[ordina]";
   
   $q=mysqli_query($conn,$x);

   while($row=mysqli_fetch_assoc($q)){
    $email=$row["Email"];
    $product=$row["nome_prodotto"];
   }
   ?>
   <center>
    <div id="div_principale_email">
    <form class="" method="POST" action="send.php" autocomplete="on">
      <input type="hidden" name="send">

    <div class="mb-3 div_email">
    <label for="exampleInputPassword1" class="form-label label_email">Email</label>
    <input type="email" class="form-control" name="email" style="width:400px;" required id="email" id="exampleInputPassword1" placeholder="<?php echo $email; ?>">
    </div>
        

    <div class="mb-3">
    <label for="exampleInputPassword1" class="form-label"  id="label_oggetto">Oggetto</label>
    <input type="textarea" class="form-control" style="width:400px;" name="subject" rows="4" cols="50" required placeholder="inserisci oggetto messaggio" id="exampleInputPassword1">
    </div>
      

    <div class="mb-3">
    <label for="exampleInputPassword1" class="form-label" id="label_messaggio">Messaggio</label>
    <input type="text" class="form-control" style="width:400px;" required placeholder="Ordine del prodotto:<?php echo $product;?>" name="message" id="exampleInputPassword1">
    </div>
    <button type="submit" class="btn btn-primary" id="submit_ordina">Invia</button>
  </form>
  </div>
  </center>
       



   </body>
<?php
}